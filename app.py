...
if 'my_portfolio' not in st.session_state:
    st.session_state.my_portfolio = [
        {"代號": "2330", "股數": 1000, "成本": 620.0},
        {"代號": "0050", "股數": 2000, "成本": 150.0}
    ]
...
if df_filtered.empty:
    ...
    st.dataframe(df_filtered[['代號', '名稱', '即時價位', '漲跌幅(%)', '現金殖利率(%)', '夏普值', 'Beta值']], use_container_width=True, hide_index=True)
...
if len(st.session_state.my_portfolio) > 0 and not df_all_market.empty:
   ...
    
    ...
    col_m2.metric("總未實現損益", f"+${int(total_profit):,}" if total_profit >= 0 else f"-${int(total_profit):,}", f"{round(total_roi, 2)}%")
   ...
    
    ...
    if not high_quality_profit.empty:
        for _, row in high_quality_profit.iterrows():
            st.success(f"💎 **強勢資產續抱指引**：您的庫存【{row['代號']} {row['名稱']}】目前正穩定獲利（+{row['報酬率 (%)']}%），且**夏普值高達卓越的 {row['夏普值']}**。長線建議抱緊。")
